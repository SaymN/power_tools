package ch.saymn.power_tools.recipe;
...
import ch.saymn.power_tools.init.BackpackRecipeSerializers;
import ch.saymn.power_tools.item.backpack_item;
...
public class BackpackCraftingRecipe extends ShapedRecipe {
	...
	public ItemStack getCraftingResult(CraftingInventory inventory) {
		...
		for (int index = 0; index < inventory.getSizeInventory(); ++index) {
			...
			if (!slotStack.isEmpty()) {
				...
				if (item instanceof backpack_item) {
	...
	public IRecipeSerializer<?> getSerializer() {
		return BackpackRecipeSerializers.BACKPACK;
	}
