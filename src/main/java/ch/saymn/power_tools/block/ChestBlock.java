...
public class ChestBlock extends Block implements IWaterLoggable
{
    ...
    protected static final VoxelShape BORNITE_CHEST_SHAPE = Block.makeCuboidShape(1.0D, 0.0D, 1.0D, 15.0D, 14.0D, 15.0D);
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack)
    {
        ...
        {
            ((BorniteChestTileEntity) tileentity).wasPlaced(placer, stack);

            if (stack.hasDisplayName())
            {
                ((BorniteChestTileEntity) tileentity).setCustomName(stack.getDisplayName());
            }
        }
    }
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving)
    {
        if (state.getBlock() != newState.getBlock())
        {
            ...
            if (tileentity instanceof BorniteChestTileEntity)
            {
                ((BorniteChestTileEntity) tileentity).removeAdornments();
                InventoryHelper.dropInventoryItems(worldIn, pos, (BorniteChestTileEntity) tileentity);
